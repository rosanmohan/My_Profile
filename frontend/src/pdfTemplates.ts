...
// Preview functions that return blob URL instead of downloading
export const generatePDFPreview = (data: ResumeData, template: 'classic' | 'modern'): string => {
    const doc = template === 'classic' ? createClassicPDF(data) : createModernPDF(data);
    const blobUrl = doc.output('bloburl');
    return typeof blobUrl === 'string' ? blobUrl : String(blobUrl);
};

// Refactored to separate PDF creation from saving
const createClassicPDF = (data: ResumeData): jsPDF => {
    const doc = new jsPDF();
    const margin = 20;
    let yPos = 20;
    const pageWidth = doc.internal.pageSize.getWidth();
...
const createModernPDF = (data: ResumeData): jsPDF => {
    const doc = new jsPDF();
    const leftMargin = 15;
    const rightMargin = 15;
    const pageWidth = doc.internal.pageSize.getWidth();
